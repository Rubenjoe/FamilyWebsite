...
export default function FamilyTree({ members, rootId }: FamilyTreeProps) {
  ...
  if (!rootPerson) {
    return (
      <div className="text-center py-12 text-[#1b3622] font-serif text-lg">
  ...
  return (
    <div className="w-full overflow-x-auto py-8">
      <div className="min-w-[700px] flex flex-col items-center">
        {renderTreeNode(rootPerson, members, 1, isNodeExpanded, toggleNode, matchingMembers.map(m => m.id), visited)}
      </div>
    </div>
  );
}
...
function renderTreeNode(
  ...
  return (
    <div className="flex flex-col items-center">
      {/* Couple Block */}
      <div className="flex flex-col sm:flex-row items-center gap-3 bg-white/50 p-4 rounded-2xl border border-[#1b3622]/5 shadow-sm">
        {/* Person Card */}
        <MemberCard member={person} generation={generation} isMatch={isPersonMatch} />

        {/* Marriage Connector */}
        {spouse && (
          <div className="flex sm:flex-col items-center justify-center gap-1 my-1 sm:my-0 text-[#d4af37]">
      ...
      {hasChildren && (
        <div className="flex flex-col items-center mt-4 w-full">
          ...
          <div className="w-[2px] h-6 bg-[#1b3622]/15"></div>

          {/* Expand/Collapse Toggle Button */}
          <button
            onClick={() => toggleNode(person.id)}
            className="flex items-center gap-2 bg-[#fbf9f4] border border-[#1b3622]/20 hover:border-[#1b3622] hover:bg-white text-[#1b3622] px-4 py-1.5 rounded-full text-[10px] uppercase tracking-widest font-bold shadow-sm transition-all duration-200 cursor-pointer z-10"
          >
            {isExpanded ? (
              <>
          ...
          <AnimatePresence initial={false}>
            {isExpanded && (
              ...
              >
                ...
                <div className="w-[2px] h-6 bg-[#1b3622]/15 mx-auto"></div>

                {/* Sub-tree Container */}
                <div className="flex flex-col items-stretch max-w-4xl mx-auto pl-12 pr-4 space-y-6 relative border-l border-[#1b3622]/15 ml-4 sm:ml-8 md:ml-12">
                  {children.map((child, index) => {
                    const isLast = index === children.length - 1;
                    return (
                      <div key={child.id} className="relative">
                        {/* Horizontal Line Connector */}
                        <div className="absolute left-[-49px] top-12 w-12 h-[2px] bg-[#1b3622]/15"></div>
                        {isLast && (
                          <div className="absolute left-[-49px] top-12 bottom-0 w-[2px] bg-[#fbf9f4]"></div>
                        )}
...
function MemberCard({ member, generation, isMatch }: { member: Member; generation: number; isMatch: boolean }) {
  ...
  return (
    <div
      className={`w-[260px] bg-white border rounded-xl p-4 flex flex-col gap-2 shadow-sm transition-all duration-300 ${
        isMatch 
      ...
      <div className="flex items-center gap-3">
        {member.photo_url ? (
          <img
            ...
            className="w-12 h-12 rounded-full object-cover border border-[#1b3622]/15 shrink-0"
          />
        ) : (
          <div className="w-12 h-12 rounded-full bg-[#fbf9f4] border border-[#d4af37]/50 flex items-center justify-center text-[#1b3622] font-serif text-sm font-semibold tracking-wider shrink-0 select-none">
            {initials}
          </div>
        )}
        ...
        <div className="flex flex-col min-w-0">
          ...
          <h4 className="font-serif text-[15px] font-semibold text-[#1b3622] leading-tight truncate">
        ...
        <div className="border-t border-gray-100 pt-2 mt-1">
          ...
          {member.bio.length > 55 && (
            <button
              ...
              className="text-[9px] uppercase tracking-wider font-bold text-[#d4af37] mt-1 hover:text-[#1b3622] cursor-pointer select-none transition-colors"
            >
