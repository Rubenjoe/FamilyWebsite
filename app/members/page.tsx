...
export default function MembersDirectory() {
    ...
    useEffect(() => {
        createClient()
            ...
            .select("id,name,branch,title,location,image_url")
            .eq("kind", "committee")
                ...
                if (data) {
                    setCommitteeMembers(
                        data.map((row) => ({
                            ...
                            generation: 1,
                            branch: row.branch,
                            profession: row.title || undefined,
                            location: row.location || "",
                            profilePhotoUrl: row.image_url || "/images/logo.png",
                            isAlive: true,
                        }))
                    );
                }
            ...
            {filteredMembers.length > 0 ? (
                                ...
                                <div className="space-y-1">
                                    ...
                                    <h3 className="text-xl text-[#1b3622] font-medium leading-snug">
                                        ...
                                        {member.malayalamName && (
                                            <span className="block text-sm text-gray-500 font-sans font-normal mt-1">
                                                {member.malayalamName}
                                            </span>
                                        )}
                                    </h3>
                                ...
                                <div className="space-y-1.5 pt-2.5 border-t border-gray-100">
                                    ...
                                    {member.profession && (
                                        <div className="flex items-center gap-2">
                                            <Briefcase className="h-4 w-4 text-[#1b3622] shrink-0" />
                ...
                <div className="text-center py-16 border border-dashed border-gray-200 bg-white/40 space-y-3">
                    ...
                    <p className="text-sm text-gray-400 italic">
                        No matching member record entries found matching those specific query bounds.
                    </p>
                </div>
            )}
