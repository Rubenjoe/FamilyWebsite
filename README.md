...
The Executive Committee is now organized as a year-based historical archive. Users can:

- Select a specific year to view committee members from that year
- View committee members with their roles, branches, and locations
- Filter by year with dynamic year selection from database data
- See committee members organized by their roles (President, Secretary, Treasurer, etc.)

The committee data is stored in the `committee_members` table in Supabase, separate from the general member directory.
...
### 6. Achievements / Excellence Registry
...
### 7. Evangelists / Faith and Service
...
**Note**: Gallery images are stored in Cloudinary for optimized delivery and automatic format/quality optimization. Gallery metadata remains in Supabase.
...
| `/members` | Executive committee (year-based archive) |
| `/achievers` | Achievements / excellence archive |
| `/obituary` | Memorial and obituary area |
| `/gallery` | Photograph archive (Cloudinary) |
| `/events` | Family events |
| `/contact` | Contact / communication |
| `/dashboard` | Dashboard entry point |
| `/admin` | Administration area |
| `/admin/members` | Member management |
| `/admin/committee` | Executive committee management (year-based) |
| `/admin/events` | Event management |
| `/admin/gallery` | Gallery management (Cloudinary) |
...
- **Supabase Storage** for managed image files (heritage photos)
- **Supabase SSR** support for Next.js server/client integration
- **Cloudinary** for gallery image storage and delivery
...
Next.js App Router
   ...
   └── Admin pages
     ...
     ▼                    ▼
PostgreSQL            Storage
 family/admin data     heritage photos
 gallery metadata
     │                    │
     └────────┬───────────┘
              ▼
...
Cloudinary is used for Gallery image storage and delivery, providing automatic image optimization, format conversion, and responsive image delivery.
...
Cloudinary Storage
    ↓
Cloudinary public_id + secure_url
    ↓
Supabase gallery_records (metadata)
    ↓
Public Gallery Page
    ↓
...
- **Automatic optimization**: Automatic format selection (WebP, AVIF) and quality optimization
- **Responsive delivery**: Automatic image sizing based on device and viewport
- **CDN delivery**: Global CDN for fast image loading worldwide
- **Transformation API**: On-the-fly image transformations
- **Year-based organization**: Gallery images organized by year in Cloudinary folders (e.g., `gallery/2026/`)

### Cloudinary vs Supabase Storage

| Feature | Cloudinary | Supabase Storage |
|---------|-----------|------------------|
| Purpose | Gallery images only | Heritage photos, member photos, etc. |
| Optimization | Automatic | Manual |
| CDN | Built-in | Via Supabase |
| Transformations | Built-in API | Limited |
| Cost | Free tier available | Included in Supabase |

### Important: Separate Systems

Cloudinary is **only** used for new Gallery uploads. Existing image systems remain unchanged:

- **Member photos**: Supabase Storage or local files
- **Committee photos**: Supabase Storage or local files  
- **Achiever photos**: Supabase Storage or local files
- **Evangelist photos**: Supabase Storage or local files
- **Obituary photos**: Supabase Storage or local files
- **Heritage photos**: Supabase Storage (`heritage-photos` bucket)
- **Gallery photos**: Cloudinary (new uploads only)

This separation ensures existing functionality continues working while providing optimized delivery for the gallery.
...
The application uses Supabase and Cloudinary. A typical Next.js client-side setup will require values similar to:
...
Cloudinary is used for Gallery image storage and delivery. Follow these steps to configure:

1. **Create or use a Cloudinary account**
   - Sign up at cloudinary.com or use an existing account
   - Navigate to your Cloudinary dashboard

2. **Find your Cloudinary credentials**
   - **Cloud Name**: Found in your Cloudinary dashboard URL or settings
   - **API Key**: Found in Account Settings → API Keys
   - **API Secret**: Found in Account Settings → API Keys (server-side only)

3. **Configure an upload preset** (for unsigned uploads)
   - Go to Settings → Upload
   - Create an upload preset with your desired settings
   - Note the preset name for the `NEXT_PUBLIC_CLOUDINARY_UPLOAD_PRESET` variable

4. **Add environment variables locally**
   - Create a `.env.local` file in the project root
   - Add the Cloudinary variables shown above
   - **CRITICAL**: `CLOUDINARY_API_SECRET` must NOT be prefixed with `NEXT_PUBLIC_`
   - This ensures the secret never reaches the browser

5. **Configure production environment variables**
   - In Vercel: Project → Settings → Environment Variables
   - Add all the Cloudinary variables
   - Ensure `CLOUDINARY_API_SECRET` is available only to server-side code
   - Never add it as `NEXT_PUBLIC_CLOUDINARY_API_SECRET`

6. **Verify Gallery upload**
   - Navigate to `/admin/gallery` as an authorized admin
   - Try uploading a gallery photo
   - Verify the image appears in Cloudinary and on the public gallery page
...
20260831_mark_placeholder_records.sql
20260901_gallery_records.sql
20260901_gallery_cloudinary_fields.sql
20260912_committee_members.sql
```
